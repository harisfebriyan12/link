...
@section('content')
    ...
    <div class="flex-1 p-4 sm:p-6 md:p-8">
        {{-- Page Header --}}
        <div class="mb-6">
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Kelola Data</h1>
            <p class="text-gray-600 text-sm sm:text-base">Kelola semua data dengan mudah</p>
        </div>

        {{-- Controls Section --}}
        <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-4 sm:p-6 mb-6">
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                <form action="{{ route('cards.index') }}" method="GET" class="relative w-full sm:flex-1 sm:max-w-md">
                    ...
                    @if(request('search'))
                        <div class="absolute inset-y-0 right-0 pr-3 flex items-center">
                            <a href="{{ route('cards.index') }}" class="text-gray-400 hover:text-gray-600">
    ...
    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <tbody class="divide-y divide-gray-100 text-gray-700" id="tableBody">
                        @forelse ($cards as $index => $card)
                            <tr class="hover:bg-green-50/50" data-searchable="{{ $card->id }}">
                                ...
                                <td class="px-4 py-3 font-semibold text-gray-900 text-sm">{{ $card->judul }}</td>
                                <td class="px-4 py-3 text-sm text-gray-600 max-w-xs hidden sm:table-cell">
                                    <p class="line-clamp-2">{{ \Illuminate\Support\Str::limit($card->deskripsi, 60) }}</p>
                                    @if(strlen($card->deskripsi) > 60)
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('cards.destroy', $card->id) }}" method="POST" class="inline-block" onsubmit="event.preventDefault(); confirmDelete('{{ $card->judul }}', this);">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <div id="edit-modal-{{ $card->id }}" class="fixed inset-0 z-50 bg-black bg-opacity-60 flex items-center justify-center hidden backdrop-blur-sm">
                                <div class="bg-white w-full max-w-md p-4 sm:p-6 rounded-lg shadow-xl relative max-h-[90vh] overflow-y-auto">
                                    ...
                                    <form action="{{ route('cards.update', $card->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4" id="edit-form-{{ $card->id }}">
                                        @csrf
                                        @method('PUT')
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <textarea name="deskripsi" rows="4"
                                                      class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-green-500 focus:border-transparent bg-gray-50 resize-none text-sm" required>{{ old('deskripsi', $card->deskripsi) }}</textarea>
                                        </div>
                                        <div>
        ...
        <div id="add-modal" class="fixed inset-0 z-50 bg-black bg-opacity-60 flex items-center justify-center hidden backdrop-blur-sm">
            <div class="bg-white w-full max-w-md p-4 sm:p-6 rounded-lg shadow-xl relative max-h-[90vh] overflow-y-auto">
                ...
                <form action="{{ route('cards.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4" id="add-form">
                    @csrf
                    <div>
                        <label class="block text-gray-700 text-sm font-semibold mb-1">Judul</label>
